...
def process_query():
    ...
    max_new_tokens = 500
    performance = "test"
    response_text = run_generation(user_text, top_p, temperature, top_k, max_new_tokens, performance)

    def generate():
        yield response_text.encode()
        time.sleep(1)
...
def predict():
    ...
    #response_text = run_generation(text, top_p, temperature, top_k, max_new_tokens, performance)
    ...
    return Response(run_generation(text, top_p, temperature, top_k, max_new_tokens, performance), mimetype='text/event-stream')
